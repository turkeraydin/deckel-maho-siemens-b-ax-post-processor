/**
  Copyright (C) 2023 by Fustech 360
  All rights reserved.

  Kurutlu Deckel Maho 810D B ax post processor configuration.

  $Revision: 00001 $
  $Date: 2023-10-03 20:37:00 +0300  $
  
  
*/

description = "KURUTLU Deckel Maho 810D B ax Post";
vendor = "FUSTECH 360";
vendorUrl = "http://www.fustech360.com";
legal = "Copyright (C) 2023 - by Fustech 360";
...
maximumCircularSweep = toRad(270.001);;;;;
...
function onOpen() {
  ...
  writeBlock("G0 SUPA Z609.5 D0");
  writeBlock("G90 D1");
  writeBlock("G0 SUPA X280 Y270 ");
  writeBlock("G90 D1");
  //writeBlock("G0 G? B?" ); 
...
function onSection() {
  ...
  if (insertToolCall || newWorkOffset || newWorkPlane) {
    ...
    retracted = true;
    writeBlock(gMotionModal.format(94), "G0", "SUPA", "Z609.9" + machineConfiguration.getRetractPlane(), "D0"); // retract
    zOutput.reset();
    writeBlock("G90 D1" );
    writeBlock("G0 SUPA X280 Y270 ");
       writeBlock("G90 D1");
...
//	}
   ...
   // writeBlock("G0 SUPA Z609.5 D0");// SOLDAKI DENDENI SATIRIN BASINA YAZARSAN AYNITAKIMLI SITRATEJIDE Z GERI KACMAZ
    writeBlock("G90 D1"); 
    ...
    if (workOffset <= 53) {
      writeBlock("G0", gFormat.format(-2) + " B0" ); // G54->G59
    } else {
      writeBlock("G0", gFormat.format(workOffset) + " B0"); // G54->G59
    }
...
//writeBlock(gPlaneModal.format(17));;;;;
  ...
  if (isFullCircle()) {
    ...
    case PLANE_XY:
      ...
      writeBlock(gPlaneModal.getCurrent(), gMotionModal.format(clockwise ? 2 : 3), iOutput.format(cx - start.x, 0), jOutput.format(cy - start.y, 0), getFeed(feed));
      break;
    case PLANE_ZX:
      if (radiusCompensation != RADIUS_COMPENSATION_OFF) {
      ...
      writeBlock(gPlaneModal.getCurrent(), gMotionModal.format(clockwise ? 2 : 3), iOutput.format(cx - start.x, 0), kOutput.format(cz - start.z, 0), getFeed(feed));
      break;
    case PLANE_YZ:
      if (radiusCompensation != RADIUS_COMPENSATION_OFF) {
      ...
      writeBlock(gPlaneModal.getCurrent(), gMotionModal.format(clockwise ? 2 : 3), jOutput.format(cy - start.y, 0), kOutput.format(cz - start.z, 0), getFeed(feed));
      break;
    default:
      linearize(tolerance);
    }
...
writeBlock("G90 D1");
    ...
    writeBlock(gMotionModal.format(94), "G0 SUPA", "X280 Y27" + machineConfiguration.getRetractPlane(), " "); // retract
    zOutput.reset();
